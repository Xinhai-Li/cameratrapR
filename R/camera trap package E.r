...
#'
#'  attach(trapresult)
#'  trapresult = trapresult[trapresult$Species=="Wild boar", ]
...
#'
#'## simulation
#'# (sim.out = simuCamtrap(trapresult, ind = 10, iteration = 1))
#'
#' @export
#'
...
simuCamtrap = function(x, detect = 50, bearing = runif(camera.N, 0, 2*pi), # detect = 0.200 / (40000/360) for latlon
  ...
  ind    = known[as.character(known$Species) == as.character(x$Species[1]), 'Ind']
...
# N.cam = length(unique(trapresult$Lat))
# N.det = length(unique(trapresult[!trapresult$Group_size == 0,2]))
# results = predictCamtrap(sim.out, trapresult, plot=T)

# Table = data.frame(Species = "野猪", Survey_area = N.cam, Act_area = N.det, Pop_size = results[2], CI_L = results[1], CI_H = results[3])
# write.csv(Table, "d:/population_size.csv", row.names = F)
...
#' @examples
#'
#'  chain = read.csv("D:/database/cameratrap/footprintchain.csv", header=T);head(chain)
#'  chain = chain[1:3000,]
#'  chain = droplevels(chain)
#'
#'  attach(chain)
#'  HOME = homeRange(chain);HOME
#'
#' @export
#'
...
#' @examples
#'  attach(chain)
#'  footprintlength = chainLength(chain); footprintlength
#' @export
